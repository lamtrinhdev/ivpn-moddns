...
func TestSign_ECDSA(t *testing.T) {
	cert, key, err := createTestECDSACertificate()
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, testDetach := range []bool{false, true} {
		...
		if err := toBeSigned.AddSigner(cert, key, SignerInfoConfig{}); err != nil {
			t.Fatalf("Cannot add signer: %s", err)
		}
		if testDetach {
			toBeSigned.Detach()
		...
		if !p7.Signers[0].DigestEncryptionAlgorithm.Algorithm.Equal(oidSignatureECDSAWithSHA256) {
			t.Fatalf("Expected ECDSA-with-SHA256 OID, got %v", p7.Signers[0].DigestEncryptionAlgorithm.Algorithm)
		}
		if err := p7.Verify(); err != nil {
			t.Fatalf("Cannot verify our signed data: %s", err)
		}
	}
}
...
func createTestECDSACertificate() (*x509.Certificate, *ecdsa.PrivateKey, error) {
